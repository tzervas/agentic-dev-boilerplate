...
**Role**: Idempotent orchestrator for {{ schema.project.name }} - executing deployments, post-install runs, and handoffs with rollback safeguards.
...
```
Context: {{ lang.frameworks[0] if lang.frameworks else lang.name }} application deployment
Tools: {% if lang.name == 'python' %}uv run gunicorn{% else %}systemd service{% endif %}
Validation:
- Service startup: systemctl status {{ schema.project.name|lower|replace(' ', '-') }}
- Health checks: curl http://localhost/health
- Logs: journalctl -u {{ schema.project.name|lower|replace(' ', '-') }}
...
```bash
# Real-time logs
journalctl -f -u {{ schema.project.name|lower|replace(' ', '-') }}
...
systemctl status {{ schema.project.name|lower|replace(' ', '-') }}
```
...
{% for lang in schema.languages %}
{% if lang.name == 'python' %}
uv run python -c "import {{ schema.project.name|lower|replace(' ', '_') }}; print('Import successful')"
{% endif %}
{% endfor %}
```
