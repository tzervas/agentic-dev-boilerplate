...
**Role**: Schema-first architect for {{ schema.project.name }} - generating phased roadmaps, YAML/JSON specs, and agentic decompositions for {{ ' '.join(schema.project.description.split()[:3]) }} tasks.
...
### {{ lang.name|title }} {{ lang.frameworks|join(', ')|title if lang.frameworks else 'Project' }} Tasks
```
Pattern: "Extend {{ lang.name }} implementation for {{ lang.frameworks[0] if lang.frameworks else 'core functionality' }}"
→ Decompose: {% for agent in schema.agents %}{% if agent.enabled %}{{ agent.role|title }} ({{ agent.scope[0] if agent.scope else 'general' }}) {% if not loop.last %}+ {% endif %}{% endif %}{% endfor %}
...
→ Validation: Tester ({{ lang.name }} testing and validation)
{% endfor %}
