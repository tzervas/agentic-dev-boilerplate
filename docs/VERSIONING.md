...
`agentic-dev-boilerplate` is **published on PyPI**, which makes it one of a small number of repos in this
...
A dependent on `agentic-dev-boilerplate = "1.1"` accepts anything `>=1.1.0, <2.0.0`. Shipping a
...
| Change                        | Bump      | Example |
| ----------------------------- | --------- | ------- |
| `fix:`                        | PATCH     | 1.1.4 → 1.1.5 |
| `feat:`                       | MINOR     | 1.1.4 → 1.2.0 |
| `feat!:` / `BREAKING CHANGE:` | **MAJOR** | 1.1.4 → **2.0.0** |
...
Consumers pin the **major**: `agentic-dev-boilerplate = "1.1"` tracks every compatible release up to but
excluding `2.0.0`. Pinning an exact patch (`"1.1.4"`) in a dependency example is a mistake —
...
- `pyproject.toml`
- `src/agentic_dev_boilerplate/__init__.py`
- `project-schema.yaml`
...
consumable; only a PyPI upload produces an artifact a dependent can resolve. For this
project the two currently **disagree**:

| | Version |
| --- | --- |
| `pyproject.toml` in this repo | `1.1.4` |
| Newest git tag | `v1.1.2` |
| **PyPI `agentic-dev-boilerplate`** | **`1.1.2`** |

`1.1.4` is tagged but **has never been published to PyPI**. Anyone reading the tag list
would reasonably conclude it shipped. It did not — `pip install agentic-dev-boilerplate` still resolves `1.1.2`.
...
5. **Publishing to PyPI is a separate, deliberate step.** It is not automatic, and until it
